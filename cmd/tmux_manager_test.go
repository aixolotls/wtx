...
func TestTmuxMouseEnabledForCurrentTerminal(t *testing.T) {
	...
	t.Run("disabled for cmux TERM_PROGRAM", func(t *testing.T) {
		t.Setenv("TERM_PROGRAM", "cmux")
		t.Setenv("TERM", "xterm-256color")
		if tmuxMouseEnabledForCurrentTerminal() {
			t.Fatalf("expected cmux TERM_PROGRAM to keep tmux mouse mode disabled")
		}
	})
	...
	t.Run("disabled via TERM fallback for cmux", func(t *testing.T) {
		t.Setenv("TERM_PROGRAM", "")
		t.Setenv("TERM", "xterm-cmux")
		if tmuxMouseEnabledForCurrentTerminal() {
			t.Fatalf("expected TERM fallback to keep tmux mouse mode disabled for cmux")
		}
	})
